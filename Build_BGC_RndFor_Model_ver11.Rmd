...
X <- fread("./inputs/WNA_8000m_HexPts_BGC_Normal_1961_1990MSY.csv", stringsAsFactors = FALSE, data.table = FALSE)#read in historic period
#  Y <- fread("./inputs/WNA_8000m_HexPts_BGC_15GCM-Ensemble_rcp45MSY.csv", stringsAsFactors = FALSE, data.table = FALSE)#read in future ensemble period
# # 
#  X1 <- rbind (X,Y)
...
X1 <- addVars(X1)
modelvars <- colnames(X1)
write.csv(modelvars, "./outputs/AllVar_WNABGCv11.csv", row.names = FALSE)
#modelvars <- read.csv("./outputs/Final27Var_WNABGCv11.csv")
#X1 <- X1 %>% dplyr::select(BGC, modelvars$x) %>% drop_na(BGC)
...
```{r test machine learning for variables, echo=FALSE}

#    BGCmodel <- ranger(BGC ~ ., data = X3, #do.trace = 10,
#                            num.trees = 101,   importance = "impurity", splitrule = "extratrees", seed = 12345, write.forest = TRUE, classification = TRUE)#strata=BGC, sampsize= c(500),
# 
# BGCmodel
# 
...
X0 <- fread("./inputs/WNA_2k_HexPts_BGC_Normal_1961_1990MSY.csv", stringsAsFactors = FALSE, data.table = FALSE)
# X1 <- fread("./inputs/WNAv11_2k_HexPts_Normal_1961_1990MSY.csv", stringsAsFactors = FALSE, data.table = FALSE)
# X1_BGC <- fread("./inputs/WNAv11_2k_HexPts_BGC.csv", stringsAsFactors = FALSE, data.table = FALSE)
# X1 <- left_join(X1, X1_BGC,   by = c("ID1" = "FID"))
# X1 <- X1[!(X1$BGC == ""),]
...
#fread("./cLHS/WNA_Training_20_VARS_new.csv")


BGCmodel <- ranger(BGC ~ ., data = X2_final[,-c(1)], #do.trace = 10, 
...
BGCmodel
save(BGCmodel,file="./outputs/WNAv11_WHM1_VAR_SubZone_ranger.Rdata")
