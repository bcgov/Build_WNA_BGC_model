...
vs9 <- c( "DD5_at",   "DD5_sm",   "DD5_sp", #"DD5_wt","Tave_at",
            "Eref_sm",   "Eref_sp",
           "Tmin_sp","Boreal", #Tmin_wt","EMT","MCMT",
...
mod1 <- glm(target ~ CMD + DD5_at, data = X0, family = "binomial")
anova(mod1, test = "Chisq")
mod2 <- glm(target ~ CMD + DD5_at + CMD:DD5_at, data = X0, family = "binomial")
anova(mod2, test = "Chisq")
...
library(iRF)
t2 <- iRF(x = xmat, y = as.factor(X0$target),ntree = 101, n.core = 1, bootstrap.forest = F, n.iter = 3, 
...
X0 <- X0[,c("BGC",vs14), with = F] ## select the variable set here
X0 <- X0 %>% mutate_if(is_integer,as.numeric)
#X0 <- as.data.frame(X0)
...
BGC_model <- rand_forest(trees = 101) %>%# specify that the model is a random forest
...
save(BGCmodel.tidy,file= paste0("./BGC_models/WNAv12_Zone_", covcount, "_Var_tidyrfCoast.Rdata"))
