...
X0[,V1 := NULL]
setnames(X0, old = "ID2", new = "BGC")
IDVars <- X0[,.(ID1,Latitude,Longitude)]
X0 <- X0[,c("ID1","BGC",vs9), with = F] ## select the variable set here
...
BGC_train <- X0[!BGC %in% BGC_Nums[Num < 25, BGC],]
...
fwrite(minDist,"ClosestCentroid35VarReduced.csv")
...
BGC_train[,mahDist := NULL]
BGC_clean <- recipe(BGC ~ CMD+DD5+MCMT+Tmin_wt+MSP, data = BGC_train) %>%
  step_classdist(all_predictors(), class = "BGC", 
                 pool = F, mean_func = mean, prefix = "mahdist_")

mahDist <- prep(BGC_clean,training = BGC_train)
mahOut <- bake(mahDist, new_data = BGC_train)
mahAll <- as.data.table(mahOut)
mahAll <- melt(mahAll[,!c("DD5","MSP","Tmin_wt","CMD","MCMT")], id.vars = c("ID1","BGC"))
setnames(mahAll, old = c("variable","value"), new = c("Centroid","MD"))
minDist <- mahAll[, .SD[which.min(MD)], by = ID1]
rm(mahAll)
gc()
minDist[,Centroid := gsub("mahdist_","",Centroid)]
minDist[,Same := BGC == Centroid]
