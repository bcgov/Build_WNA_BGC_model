...
X0 <- X0 %>% select(-ID1) %>% dplyr::rename(ID1 = V1) %>% dplyr::rename(BGC = ID2)#tibble::rownames_to_column("ID1") #
#X0 <- fread("./inputs/BC_2000m_Pts_Normal_1961_1990MSY_reduced.csv")
#X0 <- fread("./inputs/WNA_4k_HexPts_BGC_Normal_1961_1990MSY.csv")
Xreduce <- fread("./inputs/ClosestCentroid35Var.csv")
...
#
X0 <- Xjoin  %>% filter(Same == "TRUE") %>% dplyr::select(-Elevation, -Same, -Centroid, -MD, -TSA, -Dist_Code)## %>% dplyr::rename(BGC = ID2)-ID1,-Latitude, -Longitude
...
Xoutlier <- X0 %>% select(ID1, BGC, CMD.total, DD5_sp, MSP, MCMT)%>% mutate_if(is_integer,as.numeric) 
...
XAll <- rbind(X0_small,X0_OK,X0_big) %>% select(-ID1)
...
save(BGCmodel2, file = paste0("./BGC_models/WNAv12_Subzone_", covcount, "_Var_ranger_MuchReduced2.Rdata"))
