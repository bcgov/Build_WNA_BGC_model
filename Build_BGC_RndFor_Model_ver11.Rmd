...
vs8 <- c("Tave_at","MWMT", "DD5_at",   "DD5_sm",   "DD5_sp", "DD5_wt",
         ...
         "CMD_sp","CMD_at", "CMD.def", "CMDMax","CMD","CMD.total","SHM","AHM",
          "Tmax_sp", "Tmax_sm", 
          "bFFP",  "eFFP",  "NFFD", 
          "PAS_wt",  "PAS_at",  "PAS_sp", "PPT_wt","PPT.dormant","PAS_sm")#, "TD")

vs9 <- c( "DD5_at",   "DD5_sm",   "DD5_sp", #"DD5_wt","Tave_at",
            "Eref_sm",   "Eref_sp", "TD",
           "Tmin_sp", #Tmin_wt","EMT","MCMT",
...
X0 <- X0[,c("ID1","BGC",vs16), with = F] ## select the variable set here
...
temp <- X0[,.(BGC,DD5,CMD,MSP,MCMT,Tmin_wt)]
...
BGC_train <- X0[!BGC %in% zeroVar & !BGC %in% BGC_Nums[Num < 10, BGC],.(ID1,BGC,DD5,CMD,MSP,MCMT,Tmin_wt)]
##distDat <- classDist(BGC_train[,!"BGC"],BGC_train$BGC)
BGCs <- unique(as.character(BGC_train$BGC))

##simple mahalanobis distance
BGC_train[,mahDist := mahalanobis(.SD, center = colMeans(.SD), cov = cov(.SD)), 
          .SDcols = c("DD5","CMD","MSP","MCMT"), by = BGC]
BGC_train <- BGC_train[mahDist < 5,]
out <- IDVars[BGC_train, on = "ID1"]

# mahDist <- foreach(b = BGCs, combine = rbind) %do% {
#   cat(b," ")
#   temp <- BGC_train[BGC == b,.(BGC,DD5,CMD,MSP,MCMT)]
#   centroid <- colMeans(temp[,!"BGC"])
#   mdist <- mahalanobis(temp[,!"BGC"],center = centroid, cov = cov(temp[,!"BGC"]))
# }

##this doesn't really do what we want
BGC_clean <- recipe(BGC ~ ., data = BGC_train) %>%
  step_classdist(DD5,CMD,MSP,Tmin_wt, class = "BGC", 
                 pool = F, mean_func = mean, prefix = "mahdist_")

mahDist <- prep(BGC_clean,training = BGC_train)
mahOut <- bake(mahDist, new_data = BGC_train)
