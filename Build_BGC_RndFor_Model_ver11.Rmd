...
X0 <- addVars(X0)
X0 <- X0[,c("BGC",vs6), with = F]

X0[,Num := .N, by = .(BGC)]
X0_OK <- as.data.table(X0[Num > 100 & Num < 500,])
X0[,LogNum := log(Num,base = 10)]
X0[Num <= 100, New := rescale(LogNum, to = c(50,100))]
X0[Num >= 500, New := rescale(LogNum, to = c(500,1000))]
...
BGCmodel
save(BGCmodel,file="./BGC_models/WNA_Subzone_35Var_extratree.Rdata")
...
fname <- "35_VAR2"
model = "_ranger_6Oct2020"
write.csv (BGCmodel$variable.importance, file= paste("./outputs/",fname,"_Importance_extratrees",model,".csv",sep=""))
