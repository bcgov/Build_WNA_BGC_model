...
hexpoly <- as.data.table(hexpoly) %>% st_as_sf()
hexZone <- X1.pred[hexpoly, on = c(ID1 = "hex_id")] 
hexZone <- st_transform(hexZone,3005) %>% st_cast() %>% select(BGC, geom)
...
st_write(t2, dsn = paste0("./outputs/", region, "_", "SubZoneMap_hex400_dissolved2.gpkg"), layer = paste0(region, "_", timeperiod, "_", covcount, "_ranger_NoMonth3"), driver = "GPKG", delete_layer = TRUE)
