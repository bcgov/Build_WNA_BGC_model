...
### This chunk can be run a single time to build grid of points with elevation and save for reuse.
...
fwrite(coords2, "WNA_2km_grid_BGC.csv")
toc()
###260 seconds
...
### Use climR to add downscaled climate data to points
```{r Add downscaled climate variables to each point}
library(climRdev)
library(terra)
coords <- fread("WNA_2km_grid_BGC.csv")%>% dplyr::rename(long = x, lat = y)
setcolorder(coords, c("BGC", "long","lat","elev","id"))
...
### normal = "" is for setting the PRISM scale normal_na is 4km and normal_bc is 80m
normal <- climRdev::normal_input_postgis(dbCon = dbCon, normal = "normal_na", bbox = thebb, cache = TRUE) ##this takes a couple mins the first time it downloads
vars_needed <- "list_variables()"
## if known choose shorter variable list for downscaling
...
setDT(clim_vars)
clim_vars <- clim_vars[!is.nan(PPT05),]
```
