...
trainData <- removeOutlier(as.data.frame(trainData), alpha = .025, numIDvars = 1) |>
  Cache(.)
```
...
```{r balance}
BGCbalance_recipe <- recipe(BGC ~ ., data =  trainData) |>
...
setDT(X_balanced)
# BGC_Nums <- X_balanced[,.(Num = .N), by = BGC]   ## for inspection
```
