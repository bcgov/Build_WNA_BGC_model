...
load("./BGC_models/WNAv12_Subzone_11_Var_ranger_6July21.Rdata")
BGCmodel <- BGCmodel2
...
changeIDs <- BGC[BGC == origUnit & BGC.pred == changeUnit,ID1]
datChange <- Y[ID1 %in% changeIDs,]
datChange <- melt(datChange,id.vars = "ID1", variable.name = "ModVar")
datChange[,Unit := "AOIChange"]
datChange[,ID1 := NULL]
modVars <- c("Period","ID2", model_vars$rowname)

datFull <- fread("~/BGC-Climate-Summaries/ClimateBC_Out/BC_v12_Points_1901-2020MSY.csv")
datFull <- datFull[Year > 1959 & ID2 %chin% c(origUnit,changeUnit),]
gc()
datFull[,`:=`(Latitude = NULL,Longitude = NULL,Elevation = NULL)]
datFull[,Period := fifelse(Year > 1990,"Current","Historic")]
datAvg <- datFull[,lapply(.SD,mean), by = .(Period,ID2,ID1), .SDcols = -"Year"]
datAvg <- addVars(datAvg)
datAvg <- datAvg[,..modVars]
datAvg <- melt(datAvg, id.vars = c("Period","ID2"),variable.name = "ModVar")
datAvg <- datAvg[Period == "Historic",]
datAvg[,Period := NULL]
...
st_write(t2, dsn = paste0("./outputs/", covcount, "_", "Subzone_Map_hex400_dissolved_reduced6July21.gpkg"), layer = paste0(region, "_", timeperiod, "_", region ,"_", covcount, "_vars_ranger"), driver = "GPKG", delete_layer = TRUE)
